...
#define   MESH_SSID       "GKN-CEDIS-1"
#define   MESH_PASSWORD   "RTV-20.22"
...
//StaticJsonBuffer<100> jsonBuffer; //Json 5
StaticJsonDocument<100> jsonBuffer; //Json 6
//JsonObject& params = jsonBuffer.createObject(); //Json 5
DynamicJsonDocument  params(1024);  //Json 6
...
void readSerial()
{
	  ...
	  if (Serial.available())
	  {
		  ...
		  //DynamicJsonBuffer jsonBuffer(150); Json 5
      DynamicJsonDocument  dataJSON(150);
		  //JsonObject &dataJSON = jsonBuffer.parseObject(data);  //Json 5
      //JsonObject &dataJSON = deserializeJson(jsonBuffer, data);
      //JsonObject dataJSON;
      DeserializationError error = deserializeJson(jsonBuffer, data); //Json 6
      if (error)
        return;
        
          //if the data is of a JSON string, --> forward it to other nodes
          //if (dataJSON.success()) //Json 5
          if (!dataJSON.isNull())        //Json 6
          {
...
void receivedCallback(uint32_t from, String &msg) 
{
  ...
  //Serial.printf("startHere: Received from %u msg=%s\n", from, msg.c_str());
	...
	// added by yoppy
	//DynamicJsonBuffer jsonBuffer(100);  //Json 5
  DynamicJsonDocument  jsonBuffer(100); //Json 6
	//JsonObject &message = jsonBuffer.parseObject(msg);  //Json 5
  //JsonObject message;
  DeserializationError error = deserializeJson(jsonBuffer, msg);  //Json 6
  if (error)
    return;

	//if (message.success())  //Json 5
  if (!error)               //Json 6
	{
	
		if(jsonBuffer.containsKey("query")) {
			JsonArray queryParam = jsonBuffer["query"];			
			Serial.print("[ESP] Number of query params:");
			Serial.println (queryParam.size());			
			
			//StaticJsonBuffer<80> jsonBuffer;
      StaticJsonDocument<80> msg;
			...
			String str;
			//msg.printTo(str); /Json5
      serializeJson(msg,str); //Json 6
			Serial.println(str);			
		...
		else if(jsonBuffer.containsKey("set")) {
			Serial.println("[ESP] <found a 'set' key.>");
						
			//JsonObject& setParams = jsonBuffer["set"];  //Json 5
      JsonObject setParams = jsonBuffer["set"];  //Json 6
		...
		else if(jsonBuffer.containsKey("query-reply")){
			Serial.println("[ESP] <found a 'query-reply' key.>");
		}
		else if(jsonBuffer.containsKey("set-reply")){
